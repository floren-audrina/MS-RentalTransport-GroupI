...
class GatewayService:
    name = 'gateway'
    rental_rpc = RpcProxy('jayamahe_easy_ride_jakarta')
    ...
    def get_car(self, request):
        ...
        if result:
            ...
            return (404, self.header, json.dumps({"error": "Car not found"}))
    ...
    def get_available_cars(self, request, tanggal_mulai, tanggal_selesai):
        ...
        if not tanggal_mulai or not tanggal_selesai:
            return (400, self.header, json.dumps({"error": "tanggal_mulai and tanggal_selesai parameters are required"}))
        if not all(isinstance(field, str) for field in [tanggal_mulai, tanggal_selesai]):
            return (400, self.header, json.dumps({"error": "tanggal_mulai and tanggal_selesai must be a string so it can be parsed to date"}))
        ...
        if result_list:
            return (200, self.header, json.dumps(result_list))
        else:
            return (404, self.header, json.dumps({"error": "No available cars"}))
    ...
    def add_car(self, request):
        ...
        if not isinstance(car_list, dict):
            return (400, self.header, json.dumps({"error": "Expected a list of car details"}))
    ...
    def edit_car(self, request):
        ...
        if not isinstance(car_details, dict):
            return 400, self.header, json.dumps({"error": "Expected a dictionary of car details"})
        ...
        if car_id is None or not isinstance(car_id, int):
            return 400, self.header, json.dumps({"error": "car_id is required and must be an integer"})

        # Validate fields
        if car_brand != "-" and not isinstance(car_brand, str):
            return 400, self.header, json.dumps({"error": "car_brand must be a string"})
        if car_name != "-" and not isinstance(car_name, str):
            return 400, self.header, json.dumps({"error": "car_name must be a string"})
        if car_type != "-" and not isinstance(car_type, str):
            return 400, self.header, json.dumps({"error": "car_type must be a string"})
        if car_transmission != "-" and not isinstance(car_transmission, str):
            return 400, self.header, json.dumps({"error": "car_transmission must be a string"})
        if car_year != "-" and not isinstance(car_year, int):
            return 400, self.header, json.dumps({"error": "car_year must be an integer"})
        if car_seats != "-" and not isinstance(car_seats, int):
            return 400,self.header, json.dumps({"error": "car_seats must be an integer"})
    ...
    def delete_car(self, request, car_id):
        ...
        if result.get("status") == "error":
            ...
            return 200, self.header, json.dumps({"message": "Car deleted successfully"})
    ...
    def get_driver(self, request):
        ...
        if result:
            ...
            return 404, self.header, json.dumps({"error": "Driver not found"})
    ...
    def add_driver(self, request):
        ...
        if not isinstance(driver_list, dict):
            return 400,self.header, json.dumps({"error": "Expected a list of driver details"})
        ...
        if None in (driver_name,driver_gender,driver_age,driver_phone):
            return 400, self.header, json.dumps({"error": "All driver details fields are required and cannot be None"})
        
        if not all(isinstance(field, str) for field in [driver_name,driver_gender]):
            return 400, self.header, json.dumps({"error": "driver_name and driver_gender] must be strings"})

        if not all(isinstance(field, int) for field in [driver_age,driver_phone]):
            return 400, self.header, json.dumps({"error": "driver_age and driver_phone must be integers"})
    ...
    def edit_driver(self, request):
        ...
        if not isinstance(driver_details, dict):
            return 400, self.header, json.dumps({"error": "Expected a dictionary of driver details"})
        ...
        if driver_id is None or not isinstance(driver_id, int):
            return 400, self.header, json.dumps({"error": "driver_id is required and must be an integer"})

        # Validate fields
        if driver_name != "-" and not isinstance(driver_name, str):
            return 400, self.header, json.dumps({"error": "driver_name must be a string"})
        if driver_gender != "-" and not (driver_gender == 'M' or driver_gender == 'F'):
            return 400, self.header, json.dumps({"error": "driver_gender must be 'M' or 'F'"})
        if driver_age != "-" and not isinstance(driver_age, int):
            return 400, self.header, json.dumps({"error": "driver_age must be an integer"})
        if driver_phone != "-" and not isinstance(driver_phone, str):
            return 400, self.header, json.dumps({"error": "driver_phone must be a string"})
        if car_id != "-" and not (isinstance(car_id, int) or car_id is None):
            return 400, self.header, json.dumps({"error": "car_id must be an integer or None"})
    ...
    def delete_driver(self, request, driver_id):
        if not isinstance(driver_id, int) or driver_id <= 0:
            return 400, self.header, json.dumps({"error": "Invalid driver ID"})
        ...
        if result.get("status") == "error":
            ...
            return 200, self.header, json.dumps({"message": "Driver deleted successfully"})
    ...
    def get_booking(self, request):
        ...
        if result:
            ...
            return 404, self.header, json.dumps({"error": "Booking not found"})
    ...
    def add_booking(self, request):
        ...
        # Check if any required field is None
        if None in (tanggal_mulai, tanggal_selesai, with_driver, car_id):
            return 400, self.header, json.dumps({"error": "All booking data fields are required and cannot be None"})

        # Validate types
        if not all(isinstance(field, int) for field in [with_driver, car_id]):
        ...
        try:
            ...
            return 400, self.header, json.dumps({"error": "Invalid date format. Expected format: YYYY-MM-DD"})
    ...
    def edit_booking(self, request):
        ...
        if not isinstance(booking_details, dict):
            return 400,self.header, json.dumps({"error": "Expected a dictionary of booking details"})
        ...
        if booking_id is None or not isinstance(booking_id, int):
            return 400, self.header, json.dumps({"error": "booking_id is required and must be an integer"})
    ...
    def delete_booking(self, request, booking_id):
        ...
        if result.get("status") == "error":
            ...
            return 200,self.header, json.dumps({"message": "Booking deleted successfully"})
    ...
    def edit_provider(self, request):
        ...
        if not isinstance(provider_details, dict):
            return 400, self.header,json.dumps({"error": "Expected a dictionary of provider details"})
        ...
        if success:
            ...
            return 500, self.header, json.dumps({"error": "Failed to update provider"})